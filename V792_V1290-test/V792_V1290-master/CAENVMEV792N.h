...
typedef struct _V792N_OUTBUF_REG{
    union {
        struct {
            uint32_t data         : 12; /*int32_t = 32 bit integer, u is unsigned*/
            uint32_t overFlow     : 1;
...
class CAENVMEV792N:public CAENVMEModule{
	public:
		...
		void EventReset();
		void Write16(uint32_t regaddr, uint16_t data); /* 할당할 부분, 데이터를 cycle에 쓰는 것*/
...
/* web interface에 뒷자리에 들어가는 명령어 */
/*여기서 명령어를 register address로 넣은다음에 c파일에서 baseaddr뒤에 할당*/
enum V792N_ADDR{
